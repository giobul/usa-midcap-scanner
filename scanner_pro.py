...
def analyze_stock(ticker):
    ...
    try:
        ...
        try: poc_price = float(df.groupby(price_bins, observed=True)['Volume'].sum().idxmax().mid)
        except: poc_price = cp
        ...
        if session == 'REGULAR':
            if is_iceberg and ice_score >= 80: # Soglia alzata
                if "BUY" in ice_type: tipo = f"🧊 ICEBERG: {ice_type}"
                elif "SELL" in ice_type and ticker in MY_PORTFOLIO:
                    tipo = f"⚠️ WARNING: {ice_type}"
                    is_warning = True
            if not tipo and is_dp and dp_score >= 70: # Soglia alzata
                tipo = f"🕵️ DARK POOL: {dp_type}"
            if not tipo and z_score > 3.5 and cp > sma20: # Z-Score alzato da 2.5 a 3.5
                tipo = "🐋 INSTITUTIONAL SWEEP"
        else:
            if is_dp and dp_score >= 75:
        ...
        if tipo:
            ...
            if potenziale_gain < 0.015 and not is_warning:
                return 

            # 2. COOLDOWN ESTESO (6 ORE)
            now = datetime.now()
            if ticker in alert_history and now < alert_history[ticker] + timedelta(hours=6):
...
def main():
    ...
    print(f"🚀 Scanning {len(all_tickers)} stocks... [High Sensitivity Filter]")
    for t in all_tickers:
        analyze_stock(t)
        time.sleep(0.4) # Leggermente più lento per stabilità
    print("✅ Scan Complete.")
