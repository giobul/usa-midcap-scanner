...
ALERT_COOLDOWN_FILE = Path.home() / ".nexus_alert_cooldown_v7_9.json"
DAILY_CACHE_FILE = Path.home() / ".nexus_daily_cache_v7_9.json"

# Lock per la sincronizzazione dei thread
alert_lock = Lock()
cache_lock = Lock()

# --- DATABASE TITOLI (200 Tickers Pulito) ---
...
WATCHLIST = sorted(list(set([
    "ARM", "SMCI", "SNOW", "NET", "CRWD", "DDOG", "ZS", "PANW", "AVGO", "QCOM", "MRVL", "ON", "NXPI", "TXN", "ADI", "MCHP", "STM", "WOLF", "GFS", "INTC", "MU", "TSM", "ASML", "LRCX", "AMAT", "KLAC",
    "HOOD", "AFRM", "UPST", "SQ", "SHOP", "SE", "MELI", "U", "ROKU", "TDOC", "BILL", "ASAN", "MNDY", "TEAM", "DOCU", "OKTA", "TWLO", "DBX", "WDAY", "ADBE", "CRM",
    "RIOT", "CLSK", "HIVE", "HUT", "BITF", "CAN", "EBON", "SIGM", "IREN", "WULF", "TERA", "CIFR", "BTBT", "SDIG",
    "SPCE", "PLUG", "FCEL", "BE", "QS", "DNA", "ENPH", "SEDG", "FSLR", "RUN", "CHPT", "BLNK", "JOBY", "ACHR", "EH", "LILM", "IONQ", "QBTS", "BKSY", "PL", "LLAP",
    "CRSP", "EDIT", "BEAM", "NTLA", "PACB", "ILMN", "SDGR", "RXRX", "EXAI", "ADPT", "ABCL", "AMGN", "GILD", "BIIB", "REGN", "VRTX", "MRNA",
    "RIVN", "LCID", "NIO", "XPEV", "LI", "NKLA", "UBER", "LYFT", "DASH", "ABNB", "BKNG", "EXPE", "CVNA", "CAR",
    "ETSY", "CHWY", "W", "PINS", "SNAP", "TTD", "META", "GOOGL", "AMZN", "MSFT", "AAPL", "NFLX", "DIS", "PARA", "WBD",
    "SCHW", "JPM", "GS", "MS", "BAC", "C", "WFC", "V", "MA", "AXP", "BLK", "NU", "PAGS", "DLO", "GLBE",
    "GME", "AMC", "BB", "OPEN", "RDDT", "PENN", "WYNN", "LVS", "MAR", "HLT"
])))
...
def get_daily_stats_safe(ticker):
    ...
    try:
        ...
        print(f"❌ Errore Daily {ticker}: {str(e)[:50]}")
        return None, None, "UNKNOWN"

def calculate_nexus_v7_9_2(df_today, df_full, rvol):
...
def process_ticker(ticker, alert_history, today_start, vol_factor):
    try:
        ...
        score, vwap_val, vwap_status = calculate_nexus_v7_9_2(df_today, df, rvol)
        threshold = CONFIG['PORTFOLIO_THRESHOLD'] if ticker in MY_PORTFOLIO else CONFIG['NEXUS_THRESHOLD']
        
        if score >= threshold:
            return {'ticker': ticker, 'cp': float(df['Close'].iloc[-1]), 'score': score, 'vwap': vwap_status, 'atr': atr}
        return None
    except Exception as e:
        print(f"❌ Errore Process {ticker}: {str(e)[:50]}")
        return None
...
def main():
    ...
    if ALERT_COOLDOWN_FILE.exists():
        try:
            with open(ALERT_COOLDOWN_FILE, 'r') as f:
                data = json.load(f)
                alert_history = {k: datetime.fromisoformat(v) for k, v in data.items() if datetime.fromisoformat(v) > (datetime.now() - timedelta(days=1))}
        except: pass

    today_start = ny_now.replace(hour=0, minute=0, second=0, microsecond=0)
    vol_factor = 390 / max(((ny_now.hour - 9) * 60 + (ny_now.minute - 30)), 1)

    print(f"🚀 Nexus v7.9.2 Iron-Clad | Threads: {CONFIG['MAX_THREADS']}")
    ...
    with ThreadPoolExecutor(max_workers=CONFIG['MAX_THREADS']) as executor:
        ...
        for future in as_completed(futures):
            res = future.result()
            if res:
                ticker, cp, atr = res['ticker'], res['cp'], res['atr']
                msg = f"🧬 **NEXUS V7.9.2 ALERT**\n💎 `{ticker}` @ `${cp:.2f}`\n"
                msg += f"━━━━━━━━━━━━\n🎯 Score: `{res['score']}/100` | VWAP: {res['vwap']}\n"
                ...
                print(f"✅ ALERT: {ticker} @ {res['score']}")
